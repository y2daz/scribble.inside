	...
	public void PopulateMeshes(GameObject[] gs, Dictionary<string, Material> mats) {
		...
		for(int i = 0; i < gs.Length; i++) {
			...
			if(od.groups.Count == 1) {
                ...
                //gs[i].GetComponent<Renderer>().material.color = Color.white;
                for (int j = 0; j < gs[i].GetComponent<Renderer>().materials.Length; j++)
                {
                    gs[i].GetComponent<Renderer>().materials[j].color = Color.white;
                }
			}
		}
	}
